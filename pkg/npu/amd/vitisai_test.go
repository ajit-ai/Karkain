...
func TestBackendInterface(t *testing.T) {
	...
	if bk.Name() != "npu-amd" {
		t.Errorf("expected npu-amd, got %s", bk.Name())
	}
	var nb npu.NPUBackend = b
	if nb.Vendor() != npu.VendorAMD {
		t.Error("expected amd vendor")
	}
}
...
func TestCompile(t *testing.T) {
	...
	dev := npu.NPUDevice{Name: "XDNA", Vendor: npu.VendorAMD}
	prog, err := b.Compile(g, dev)
	if err != nil {
		t.Fatalf("compile failed: %v", err)
	}
	if prog.Vendor != npu.VendorAMD {
		t.Error("expected amd program")
	}
	if string(prog.Blob) != "vitisai:XDNA" {
...
func TestExecute(t *testing.T) {
	...
	if res.Metadata["npu_vendor"] != "amd" {
		t.Error("expected amd vendor metadata")
	}
}
