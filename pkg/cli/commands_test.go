...
func TestCLI_CheckCommand(t *testing.T) {
	// Test valid .kark file
	validFile := filepath.Join(t.TempDir(), "valid.kark")
	...
	// Test invalid .kark file (syntax error)
	invalidFile := filepath.Join(t.TempDir(), "invalid.kark")
	...
	result = CheckCommand(filepath.Join(t.TempDir(), "nonexistent.kark"), false)
	if result.ExitCode == 0 {
...
func TestCLI_BuildAndRunCommands(t *testing.T) {
	// Create a simple valid .kark file
	testDir := t.TempDir()
	simpleFile := filepath.Join(testDir, "simple.kark")
...
func TestCLI_TestRunner(t *testing.T) {
	...
	testDir := t.TempDir()
	testFile := filepath.Join(testDir, "math_test.kark")
...
func TestCLI_TestRunner_NoTests(t *testing.T) {
	...
	testDir := t.TempDir()
	noTestFile := filepath.Join(testDir, "nontest.kark")
...
func TestCLI_CheckCommand_InvalidExtension(t *testing.T) {
	...
	if result.ExitCode == 0 {
		t.Error("CheckCommand on non-.kark file should fail")
	}
	if !strings.Contains(result.Message, ".kark") {
		t.Errorf("Expected error about .kark extension, got: %s", result.Message)
	}
}

func TestCLI_RunCommand_InvalidFile(t *testing.T) {
	result := RunCommand("nonexistent.kark", codegen.NewConfig(), false)
	if result.ExitCode == 0 {
...
func TestCLI_BuildCommand_InvalidFile(t *testing.T) {
	result := BuildCommand("nonexistent.kark", "", codegen.NewConfig(), false)
	if result.ExitCode == 0 {
