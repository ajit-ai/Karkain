...
var errorRegistry = []explanation{
	...
	{"K115", "Generics v1 (both engines; Go reports it as a resolve-class diagnostic): a generic call or struct literal passes the wrong number of explicit type arguments, uses an undefined generic struct, or a generic function/struct is used without explicit type arguments (`f[int](...)`, `Point[int]{...}`).", "Pass exactly the declared number of explicit type arguments at every generic call site; type-argument inference is a future slice, so bare uses of generic functions/structs are rejected."},
	{"K145", "Native x86-64 backend: the program uses a construct outside the v1 surface (floats, maps, for-in/arrays, main with arguments, arity or int/string kind mismatches, over-deep expression nesting). Straight-line code, calls (any arity, int/string args and returns), and if/else/while/C-for loops with break/continue all compile.", "Stay within ints, strings, calls, let/print/return and comparison-loop control flow; branches beyond that surface are future slices (PE/Mach-O, Value-model ops, register allocation)."},
