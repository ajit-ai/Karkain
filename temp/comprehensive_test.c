...
Value* greet(Value* name) {
	print_value(make_string("Hello from function!"));
	print_value(name);
	return make_int(0);
}

Value* test_variables() {
	Value* x = make_int(42);
	Value* name = make_string("Karkain");
	Value* pi = make_float(3.14);
	print_value(x);
	print_value(name);
	print_value(pi);
	return make_int(0);
...
Value* test_precedence() {
	Value* result = binary_op(make_int(2), "+", binary_op(make_int(3), "*", make_int(4)));
	print_value(result);
	Value* a = binary_op(make_int(10), "-", binary_op(make_int(2), "*", make_int(3)));
	print_value(a);
	Value* b = binary_op(binary_op(make_int(10), "-", make_int(2)), "*", make_int(3));
	print_value(b);
	return make_int(0);
}

Value* test_comparisons() {
	Value* big = binary_op(make_int(10), ">", make_int(5));
	Value* small = binary_op(make_int(3), "<", make_int(7));
	Value* equal = binary_op(make_int(5), "==", make_int(5));
	Value* noteq = binary_op(make_int(5), "!=", make_int(3));
	Value* geq = binary_op(make_int(10), ">=", make_int(10));
	Value* leq = binary_op(make_int(3), "<=", make_int(5));
	print_value(big);
	print_value(small);
	print_value(equal);
	print_value(noteq);
...
Value* test_strings() {
	Value* first = make_string("Hello");
	Value* second = make_string(" World");
	Value* combined = binary_op(first, "+", second);
	print_value(combined);
	Value* len_result = karkain_len(combined);
	print_value(len_result);
	return make_int(0);
}

Value* test_while() {
	Value* i = make_int(0);
	Value* sum = make_int(0);
	while (is_truthy(binary_op(i, "<", make_int(5)))) {
		sum = binary_op(sum, "+", i);
		i = binary_op(i, "+", make_int(1));
	}
	print_value(sum);
	return make_int(0);
}
...
Value* test_if_else() {
	Value* score = make_int(85);
	if (is_truthy(binary_op(score, ">=", make_int(90)))) {
		print_value(make_string("A"));
	} else if (is_truthy(binary_op(score, ">=", make_int(80)))) {
		print_value(make_string("B"));
	} else if (is_truthy(binary_op(score, ">=", make_int(70)))) {
		print_value(make_string("C"));
	} else {
		print_value(make_string("F"));
	}
...
Value* test_arrays() {
	Value* arr = ({ Value* _arr = make_array(); array_push(_arr, make_int(10)); array_push(_arr, make_int(20)); array_push(_arr, make_int(30)); _arr; });
	Value* first = array_get(arr, make_int(0));
	Value* second = array_get(arr, make_int(1));
	Value* third = array_get(arr, make_int(2));
	print_value(first);
	print_value(second);
	print_value(third);
	return make_int(0);
...
Value* test_maps() {
	Value* m = map_set(map_set(map_set(make_map(), make_string("a"), make_int(1)), make_string("b"), make_int(2)), make_string("c"), make_int(3));
	Value* va = array_get(m, make_string("a"));
	Value* vb = array_get(m, make_string("b"));
	print_value(va);
	print_value(vb);
	return make_int(0);
}
...
Value* test_builtins() {
	Value* s = make_string("  hello world  ");
	Value* trimmed = karkain_trim(s);
	print_value(trimmed);
	Value* haystack = make_string("hello world");
	Value* found = karkain_contains(haystack, make_string("world"));
	print_value(found);
	Value* parts = karkain_split(make_string("a,b,c"), make_string(","));
	print_value(parts);
	Value* sq = karkain_sqrt(make_int(16));
	print_value(sq);
	Value* ab = karkain_abs(binary_op(make_int(0), "-", make_int(42)));
	print_value(ab);
	Value* pw = karkain_pow(make_int(2), make_int(10));
	print_value(pw);
	return make_int(0);
}

Value* test_c_interop() {
	print_value(make_string("C interop - skipped (requires top-level import)"));
	return make_int(0);
}

Value* test_matrix() {

	// Matrix declaration: m (float64) - 64-byte aligned for SIMD
	double* m;
#ifdef _WIN32
	m = (double*)_aligned_malloc(2 * 2 * sizeof(double), 64);
#else
	m = (double*)aligned_alloc(64, 2 * 2 * sizeof(double));
#endif
	if (!m) {
		fprintf(stderr, "Matrix allocation failed\\n");
		exit(1);
	}
	memset(m, 0, 2 * 2 * sizeof(double));
	// Store matrix dimensions for row-major indexing
	const int64_t m_rows = 2;
	const int64_t m_cols = 2;
	print_value(make_string("matrix declared"));
	return make_int(0);
}

Value* factorial(Value* n) {
	if (is_truthy(binary_op(n, "<=", make_int(1)))) {
		return make_int(1);
	}
	return binary_op(n, "*", factorial(binary_op(n, "-", make_int(1))));
	return make_int(0);
}

Value* test_recursion() {
	Value* f5 = factorial(make_int(5));
	print_value(f5);
	Value* f10 = factorial(make_int(10));
	print_value(f10);
	return make_int(0);
}

Value* test_nested_loops() {
	Value* i = make_int(0);
	Value* total = make_int(0);
	while (is_truthy(binary_op(i, "<", make_int(3)))) {
		Value* j = make_int(0);
		while (is_truthy(binary_op(j, "<", make_int(3)))) {
		total = binary_op(total, "+", make_int(1));
		j = binary_op(j, "+", make_int(1));
	}
		i = binary_op(i, "+", make_int(1));
	}
	print_value(total);
...
Value* test_float_ops() {
	Value* a = make_float(1.5);
	Value* b = make_float(2.5);
	Value* sum = binary_op(a, "+", b);
	Value* diff = binary_op(b, "-", a);
	Value* prod = binary_op(a, "*", b);
	print_value(sum);
	print_value(diff);
	print_value(prod);
	return make_int(0);
...
int main() {
	quantum_init();
	print_value(make_string("=== Test 1: Functions ==="));
	Value* r = add(make_int(3), make_int(7));
	print_value(r);
	Value* m = multiply(make_int(4), make_int(5));
	print_value(m);
	greet(make_string("test"));
	print_value(make_string("=== Test 2: Variables ==="));
	test_variables();
	print_value(make_string("=== Test 3: Precedence ==="));
	test_precedence();
	print_value(make_string("=== Test 4: Comparisons ==="));
	test_comparisons();
	print_value(make_string("=== Test 5: Strings ==="));
	test_strings();
	print_value(make_string("=== Test 6: While loops ==="));
	test_while();
	print_value(make_string("=== Test 7: If/else ==="));
	test_if_else();
	print_value(make_string("=== Test 8: Arrays ==="));
	test_arrays();
	print_value(make_string("=== Test 9: Maps ==="));
	test_maps();
	print_value(make_string("=== Test 10: Builtins ==="));
	test_builtins();
	print_value(make_string("=== Test 11: C interop ==="));
	test_c_interop();
	print_value(make_string("=== Test 12: Matrix ==="));
	test_matrix();
	print_value(make_string("=== Test 13: Recursion ==="));
	test_recursion();
	print_value(make_string("=== Test 14: Nested loops ==="));
	test_nested_loops();
	print_value(make_string("=== Test 15: Float ops ==="));
	test_float_ops();
	print_value(make_string("=== ALL TESTS COMPLETE ==="));
	return 0;
}
