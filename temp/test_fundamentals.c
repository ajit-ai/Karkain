...
int main() {
	quantum_init();
	Value* result = add(make_int(3), make_int(7));
	print_value(result);
	Value* i = make_int(0);
	...
	Value* val = binary_op(make_int(2), "+", binary_op(make_int(3), "*", make_int(4)));
	print_value(val);
	Value* hello = make_string("Hello");
	Value* world = make_string(" World");
	Value* msg = binary_op(hello, "+", world);
	print_value(msg);
	Value* check = binary_op(make_int(5), "!=", make_int(3));
	print_value(check);
	Value* pi = make_float(3.14);
	Value* doubled = binary_op(pi, "+", pi);
	print_value(doubled);
	Value* x = make_int(10);
	if (is_truthy(binary_op(x, ">", make_int(20)))) {
		print_value(make_string("big"));
	} else if (is_truthy(binary_op(x, ">", make_int(5)))) {
		print_value(make_string("medium"));
	} else {
		print_value(make_string("small"));
	}
