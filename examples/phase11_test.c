...
#line 1 "examples/phase11_test.kar"
int main() {
	int64_t x = 42;
	printf("%lld\n", x);

	// Matrix declaration: mat (2) - 64-byte aligned for SIMD
	void** mat;
#ifdef _WIN32
	mat = (void**)_aligned_malloc(64, 2 * 2 * sizeof(void*));
#else
	mat = (void**)aligned_alloc(64, 2 * 2 * sizeof(void*));
#endif
	if (!mat) {
		fprintf(stderr, "Matrix allocation failed\\n");
		exit(1);
	}
	memset(mat, 0, 2 * 2 * sizeof(void*));
	// Store matrix dimensions for row-major indexing
	const int64_t mat_rows = 2;
	const int64_t mat_cols = 2;
	print_value(make_int(64));
	print_value(make_int(100));
	return 0;
}
